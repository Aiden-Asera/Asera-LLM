...
export class AnalyticsService {
  ...
  private readonly MAX_HISTORY = 1000; // Keep last 1000 searches in memory
  ...
  async trackSearch(analytics: SearchAnalytics): Promise<void> {
    try {
      logger.info('Tracking search analytics:', {
   ...
   */
  async getPopularQueries(clientId: string, limit: number = 10): Promise<QueryStats[]> {
    try {
   ...
   */
  async getSearchMetrics(clientId: string): Promise<{
    totalSearches: number;
    avgResponseTime: number;
    avgResultsPerQuery: number;
    successRate: number;
    recentSearches: SearchAnalytics[];
    topPerformingQueries: QueryStats[];
  }> {
    try {
      const clientSearches = this.searchHistory.filter(s => s.clientId === clientId);
   ...
   */
  async getTrendingQueries(clientId: string, limit: number = 5): Promise<QueryStats[]> {
    try {
      const clientSearches = this.searchHistory.filter(s => s.clientId === clientId);
      ...
      const trending: Array<QueryStats & { trendScore: number }> = [];
   ...
   */
  private countQueries(searches: SearchAnalytics[]): Map<string, number> {
